...
Hardwood is three pieces joined by one contract.

```
 ┌──────────────────────────────────────────────────────────────────────────────┐
 │  contracts/                                                                  │
 │    metrics.json · widgets.json · presets.json · CONTRACT.md · fixtures/       │
 │    the single source of truth, consumed by BOTH sides, drift-checked in CI    │
 └───────────────┬───────────────────────────────────────┬──────────────────────┘
                 │                                       │
   ┌─────────────▼──────────────┐         ┌──────────────▼───────────────────┐
   │  backend/  (Python)        │  HTTP   │  ios/  (SwiftUI, iOS 17+)        │
   │                            │◄───────►│                                  │
   │  ingest → store → API      │  JSON   │  dashboard · widgets · sync      │
   └────────────────────────────┘         └──────────────────────────────────┘
```
...
Two codebases in two languages have to agree on 61 metric definitions, 12 widget payload shapes
and 9 preset dashboards. Encoding that agreement in prose alone guarantees drift, so it is
encoded in data instead:
...
* `contracts/widgets.json` — the 12 widget kinds and, for each, a typed schema of its
  configuration fields. **The iOS configuration editor is generated from this file at runtime**,
  which is why adding a widget option does not require an app release.
* `contracts/presets.json` — the 9 shipped dashboards, pre-validated against both catalogs by
  their generator.
...
bundled into the app under `ios/NBAStats/Resources/Contracts/`.
...
| `scripts/check_contracts.py` | Catalogs regenerate; presets validate; backend registry matches the catalog; the app's bundled copies are byte-identical |
